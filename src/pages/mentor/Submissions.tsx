...
export default function MentorSubmissions({
  ...
  // 1. Map mock (task-based) submissions — PRD now comes from the backend.
  const mappedSubmissions: DisplayRow[] = submissions
    .filter(sub => sub.category !== 'PRD')
    .map((sub): DisplayRow | null => {
      const task = tasks.find((t) => t.id === sub.task_id);
      const studentProf = profiles.find((p) => p.id === sub.student_id);
      const studentInfo = students.find((s) => s.user_id === sub.student_id);
      const belongsToMentor = task?.mentor_id === mentorId || task?.assigned_to === mentorId || task?.mentor_id == null;
      return belongsToMentor ? {
        id: sub.id,
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Comments Panel (mock) / Mentor Feedback (real PRD) */}
          {activeSub.isPrd ? (
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                {activeComments.length === 0 ? (
                  ...
                  activeComments.map((c) => {
                    ...
                    const isMe = c.author_id === mentorId;
                    return (
