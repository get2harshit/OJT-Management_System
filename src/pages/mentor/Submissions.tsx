...
export default function MentorSubmissions({
  ...
  const mappedSubmissions = submissions.map((sub) => {
    ...
    return {
      ...
      task_mentor_id: task?.mentor_id ?? null,
      task_assigned_to: task?.assigned_to ?? null,
    };
  });

  // Filter list by mentor assignments
  const myMappedSubmissions = mappedSubmissions.filter((sub) => {
    return sub.task_mentor_id === mentorId || sub.task_assigned_to === mentorId || sub.task_mentor_id === null;
  });

  // 2. Filter list
  const filteredSubmissions = myMappedSubmissions.filter((sub) => {
  ...
  // 3. Count categories
  const prdCount = myMappedSubmissions.filter(s => s.category === 'PRD').length;
  const videoCount = myMappedSubmissions.filter(s => s.category === 'VIDEO').length;
  const commonCount = myMappedSubmissions.filter(s => s.category === 'COMMON_TASK').length;
  const specificCount = myMappedSubmissions.filter(s => s.category === 'SPECIFIC_TASK').length;

  const activeSub = myMappedSubmissions.find(s => s.id === selectedSubId);
      ...
      <div className="flex gap-1.5 p-1 bg-zinc-850 border border-zinc-750 rounded-xl w-fit">
        {(['ALL', 'STUDENT', 'MENTOR'] as const).map((type) => (
          <button
            ...
            className={`px-4 py-1.5 text-xs font-semibold rounded-lg transition-all ${filterType === type ? 'bg-gold text-black' : 'text-gray-400 hover:text-white'
              }`}
          >
